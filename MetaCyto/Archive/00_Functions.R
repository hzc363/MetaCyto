...
autoCluster= function(preprocessOutputFolder,
  ...
  for(std in unique(inputMeta$study_id)){
    ...
    if(gatingMethod=="cluster"|length(ctr_files)==0){cutoff=apply(expr,2, findCutoff,spar=spar,minX=-Inf)}
    ...
    if(gatingMethod=="max"&length(ctr_files)>0){
      ...
      for(i in 1:length(cutoff)){
        cutoff[i]=findCutoff(expr[,i],spar=spar,minX=cutoff2[i])
      }
    }
    g=names(cutoff)%in%excludeControlParameters
    if(length(g)>0){cutoff[g]=apply(expr[,g,drop=F],2, findCutoff,spar=spar,minX=-Inf)}
...
searchCluster_ori=function(preprocessOutputFolder,
  ...
  for(std in unique(inputMeta$study_id)){
    ...
    if(gatingMethod=="cluster"|length(ctr_files)==0){
      if(assay=="CyTOF"){minX=0}else{minX=-Inf}
      cutoff=apply(expr,2, findCutoff,spar=spar,minX=minX)
      }
    #if(gatingMethod=="min"&length(ctr_files)>0){cutoff=apply(cbind(cutoff1,cutoff2),1,min)}
    if(gatingMethod=="max"&length(ctr_files)>0){
      cutoff2=apply(expr_ctr,2, function(x){quantile(x,ctrQuantile)})
...
findPeaks <- function (x,m=7){
  ...
  pks <- unlist(pks)
  if(x[1]>mean(x)){pks=union(pks,c(1))}
  pks
}

findCutoff=function(x,spar,minX=min(x)){
  library(cluster)
  ...
  m=max(round(length(dens$x)/100),7)
  peaks=findPeaks(dens$y,m=m)
  if(length(peaks)>1){peaks=peaks[dens$y[peaks]>0.1*mean(dens$y)]}
  if(length(peaks)<=1){return(max(minX,min(x)))}
  valley=sapply(2:length(peaks), function(i){
    v=findPeaks(-1*dens$y[peaks[i-1]:peaks[i]],m=m)+peaks[i-1]-1
    v=v[which.min(dens$y[v])]
    return( dens$x[v] )
  })
  valley=unlist(valley)
  if(length(valley)>=1){valley=valley[valley>=minX]}
  if(length(valley)<1){return(max(minX,min(x)))}
  s=min(1000,length(x))
  if(s<length(x)){x=sample(x,s)}
  #b=c(-Inf,sapply(seq(0,1,0.01),function(q){quantile(x,q)}),Inf)
  #x_bin=.bincode(x, breaks=b)
  D=dist( matrix(x,ncol=1) )
  sil=sapply(valley,function(v){
    cluster=1*(x>v)
    if(length(unique(cluster))==1){return(-1)}
    ss <- silhouette(cluster,D)
    return(mean(ss[, 3]))
  })
  # sil=sapply(valley,function(v){
  #   p=sum(1*(x>v))/length(x)
  #   return(-abs(p-0.5))
  # })
...
searchCluster=function(preprocessOutputFolder,
  ...
  for(std in unique(inputMeta$study_id)){
    ...
    #   for(i in 1:length(CL_label2)){
    #     rows=1:nrow(expr)
    #     for(j in 1:length(CL_label2[[i]])){
    #       L=CL_label2[[i]][j]
    #       L=strsplit(L,split="&")[[1]]
    #       R=sapply(L,function(x){
    #         m=gsub("\\+$|-$","",x)
    #         cutoff=findCutoff(expr[rows,m],spar,-Inf)
    #         if(grepl("\\+$",x)){w=which(expr[rows,m]>cutoff)}
    #         if(grepl("-$",x)){w=which(expr[rows,m]<cutoff)}
    #         #w=which(expr[,m]<quantile(expr_ind[CL[[i]],m],1) & expr[,m]>quantile(expr_ind[CL[[i]],m],0) )
    #         return(w)
    #       })
    #       if(length(L)>1){R=Reduce(intersect,R)}
    #       if(length(R)<=1){rows=sample(rows,1); break}
    #       rows=rows[R]
    #     }
    #     CL[[i]]=rows
    #   }
